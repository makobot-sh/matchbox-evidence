...
public class RecommenderTutorialFromDocs {
    ...
    public static void Evidence() {
        ...
        using (Variable.ForEach(observation)) {  
            ...
            Variable<double> affinity = (bias + Variable.Sum(products)); 
        ...
        // ***************************************************************************** //
        int numGeneratedObservations = numObservations.ObservedValue;
        ...
        for (int i = 0; i < 5; i++)
        {   
            Console.WriteLine("| {0}    {1} |", itemTraits_[i][0].ToString("F"), itemTraits_[i][1].ToString("F"));
        }
        */
